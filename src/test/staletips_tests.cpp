...
// Signet branch 00000012602f...e1c -> 0000000024ff...001.
...
// BCH branch 000000000000...d43 -> 000000000000...ffa.
...
BOOST_AUTO_TEST_CASE(staletip_testvector_deserialize)
{
    ...
    BOOST_CHECK_EQUAL(decompressed.headers.size(), 19U);
    BOOST_CHECK_EQUAL(decompressed.headers.front().hashPrevBlock.ToString(), "00000012602fde2eaf33a90523f42fb07ca854c1d26108782dc4592a80507e1c");
    BOOST_CHECK_EQUAL(decompressed.tip_hash.ToString(), "0000000024ff924ff932668d497bba7da9157559a68d9c87d2f28d22e5e4a001");
}
...
BOOST_AUTO_TEST_CASE(bch_testvector_deserialize)
{
    ...
    const auto decompressed{data.DecompressHeaders()};
    BOOST_CHECK_EQUAL(decompressed.headers.size(), 18U);
    BOOST_CHECK_EQUAL(decompressed.headers.front().hashPrevBlock.ToString(), "0000000000000000011865af4122fe3b144e2cbeea86142e8ff2fb4107352d43");
    BOOST_CHECK_EQUAL(decompressed.tip_hash.ToString(), "000000000000000001416af072f8989829f4c60a1a9658e1cec08411798e4ffa");
}
...
BOOST_AUTO_TEST_CASE(staletip_construct_from_fork)
{
    LOCK(::cs_main);
    ...
    BOOST_CHECK_EQUAL(message.m_fork_point.ToString(), fork_point->GetBlockHash().ToString());
    BOOST_REQUIRE_EQUAL(message.m_headers.size(), 2U);
    BOOST_CHECK_EQUAL(message.m_headers.front().merkle_root.ToString(), first->hashMerkleRoot.ToString());
    BOOST_CHECK_EQUAL(message.m_headers.back().nonce, tip->nNonce);
    BOOST_CHECK(message.m_have_block);

    const auto decompressed{message.DecompressHeaders()};
    BOOST_REQUIRE_EQUAL(decompressed.headers.size(), 2U);
    BOOST_CHECK_EQUAL(decompressed.headers.front().GetHash().ToString(), first->GetBlockHash().ToString());
    BOOST_CHECK_EQUAL(decompressed.tip_hash.ToString(), tip->GetBlockHash().ToString());
}

BOOST_AUTO_TEST_CASE(staletip_construct_header_count_bounds)
{
    ...
    auto construct = [&](size_t branch_length) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        return StaleTipMessage{
            StaleFork{
                .fork_point = branch.front(),
                .tip = branch.at(branch_length),
            },
            /*have_block=*/false,
        };
    };

    BOOST_CHECK_THROW(construct(0), NonFatalCheckError);
    BOOST_CHECK_NO_THROW(construct(1));
    BOOST_CHECK_NO_THROW(construct(MAX_STALETIP_HEADERS));
    BOOST_CHECK_THROW(
        construct(MAX_STALETIP_HEADERS + 1),
        NonFatalCheckError);
}

BOOST_AUTO_TEST_CASE(staletip_serialize_header_count_bounds)
{
    auto serialize = [](const StaleTipMessage& message) {
        DataStream stream;
        stream << message;
    };
    ...
    StaleTipMessage message;

    BOOST_CHECK_THROW(serialize(message), NonFatalCheckError);

    message.m_headers.resize(1);
    BOOST_CHECK_NO_THROW(serialize(message));

    message.m_headers.resize(MAX_STALETIP_HEADERS);
    BOOST_CHECK_NO_THROW(serialize(message));

    message.m_headers.resize(MAX_STALETIP_HEADERS + 1);
    BOOST_CHECK_THROW(serialize(message), NonFatalCheckError);
}
...
BOOST_FIXTURE_TEST_CASE(staletip_initialize_rejects_signet_active_tip_variant, ChainTestingSetup)
{
    LOCK(::cs_main);
    ...
    CBlockHeader variant_header{active->GetBlockHeader()};
    ++variant_header.nNonce;
    CBlockIndex* active_variant{blockman.AddToBlockIndex(variant_header, best_header)};
    active_variant->nStatus |= BLOCK_VALID_TRANSACTIONS | BLOCK_HAVE_DATA;

    StaleTipCache tips{ChainType::SIGNET};
    BOOST_CHECK(!tips.AddStaleTip(active_chain, active_variant));

    tips.Initialize(blockman, active_chain);
    BOOST_CHECK(tips.GetStaleTips(active_chain).empty());
}
