...
enum class SignetHeaderVariantResult {
 ...
 *  header spam. */
SignetHeaderVariantResult CompareSignetHeaderVariants(const CBlockIndex& candidate_tip, const CBlockIndex& existing_tip)
{
    ...
    if (candidate_tip.nHeight > existing_tip.nHeight) {
        return same_variant(&existing_tip, candidate_tip.GetAncestor(existing_tip.nHeight)) ? SignetHeaderVariantResult::PREFER_CANDIDATE : SignetHeaderVariantResult::PREFER_BOTH;
    }

    return same_variant(existing_tip.GetAncestor(candidate_tip.nHeight), &candidate_tip) ? SignetHeaderVariantResult::PREFER_EXISTING : SignetHeaderVariantResult::PREFER_BOTH;
}
...
bool StaleTipCache::Add(const CBlockIndex& stale_tip)
{
    ...
    for (auto& entry : m_tips) {
        ...
        if (m_chain_type == ChainType::SIGNET) {
            const auto variant_result{CompareSignetHeaderVariants(stale_tip, *entry.tip)};
            if (variant_result == SignetHeaderVariantResult::PREFER_EXISTING) return false;
            if (variant_result == SignetHeaderVariantResult::PREFER_CANDIDATE) {
                replace.push_back(&entry);
                continue;
            }
        }
...
void StaleTipCache::Initialize(node::BlockManager& blockman, const CChain& chain)
{
    ...
    for (const CBlockIndex* candidate : candidates) {
        if (!parents.contains(candidate)) (void)AddStaleTip(chain, candidate);
    }
}
...
bool StaleTipCache::AddStaleTip(const CChain& chain, const CBlockIndex* stale_tip, bool allow_more_work)
{
    ...
    if (m_chain_type == ChainType::SIGNET && chain.Tip() != nullptr) {
        if (CompareSignetHeaderVariants(*stale_tip, *chain.Tip()) == SignetHeaderVariantResult::PREFER_EXISTING) return false;
    }
